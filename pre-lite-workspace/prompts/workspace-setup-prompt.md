...
我已经接受 GitHub 邀请，并且能在网页上打开课程 workspace。请你帮我把它正确放进 Obsidian，让我以后能在 Obsidian 左侧文件列表里直接看到它。

请严格按以下顺序工作，不要跳步，也不要把课程文件放到桌面、下载文件夹或 `~/Projects/`。

## 1. 先确认邀请前已准备好的个人 Obsidian Vault
...
4. 如果一个 Vault 也没有找到，说明邀请前的 Obsidian 准备没有完成。告诉我这个前置条件缺失，并停下；不要在此时创建 Vault，也不要只在终端创建一个同名普通文件夹来冒充 Vault。
...
如果 GitHub 授权、Git 或克隆命令报错，请告诉我准确报错和下一步最小操作。不要要求我把密码、验证码、访问令牌、API key 或付款信息发给你。
